...
**What survives.** The witness pass attempts the **first divergence source in
program order**, skipping call sites, and promotes exactly those findings that
share that source. Everything else stays at warning tier.

| shape | first finding | second finding |
|---|---|---|
| two evaluable guards, different conditions | confirmed | warning |
| two evaluable guards, **identical** conditions | confirmed | confirmed (2 witnesses) |
| lane-environment guard **first** | warning | warning |
| lane-environment guard **second** | **confirmed** | warning |
| divergent guard inside a loop, first | warning | warning |
| **call site** first, evaluable second | warning | **confirmed** |
| first call fine, second names an absent lane | warning | warning |
...
takes it out of the gate.** Nothing in the output says so.

The *mechanism* is still not established — why the pass commits to one source
rather than trying each — and the rule that encodes this says so in its own text.
The discriminating experiment for Stage 5: three barriers under three distinct
evaluable guards. If only the first is confirmed, the pass witnesses exactly one
source; if all three are, the rule is about something else and this
characterization is wrong again.
...
Two corpora, both against reconverge 0.1.6 at `block=32`, both on a laptop.
Regenerate either table with
`scripts/conformance-summary.py <cases-dir>/conformance.json`, so these numbers
are read off the artifact rather than remembered.

### 144 cases, grouped by the rule that predicted them

| rule | provenance | held | violated | example violation |
|---|---|---:|---:|---|
| `another_divergence_source_got_the_witness` | measured | 3 | 0 |  |
| `barrier_under_evaluable_divergent_guard` | quoted | 23 | 0 |  |
| `barrier_under_guard_inside_loop` | measured | 1 | 0 |  |
| `barrier_under_lane_environment_guard` | quoted | 16 | 0 |  |
| `barrier_under_mixed_guard` | quoted | 15 | 0 |  |
| `barrier_under_truncating_cast_guard` | measured | 4 | 0 |  |
| `barrier_uniform_control` | quoted | 4 | 0 |  |
| `barrier_via_call_site` | quoted | 19 | 0 |  |
...
|---|---:|
| AnalyzerUnsupported | 69 |
| AgreementBug | 41 |
| AgreementSafe | 34 |

**144 cases: 144 predictions held, 0 violated — 0 of those about the analyzer (a quoted or measured rule), 0 about this model (an extrapolated one). 0 case(s) classified as needing a human.**

### The held-out corpus

### 60 cases, grouped by the rule that predicted them

| rule | provenance | held | violated | example violation |
|---|---|---:|---:|---|
| `another_divergence_source_got_the_witness` | measured | 4 | 0 |  |
| `collective_naming_an_absent_lane` | measured | 28 | 0 |  |
| `collective_under_guard_inside_loop` | measured | 4 | 0 |  |
| `collective_under_lane_environment_guard` | quoted | 17 | 0 |  |
| `collective_with_every_named_lane_present` | measured | 3 | 0 |  |
| `collective_with_unevaluable_mask` | quoted | 2 | 0 |  |
| `construct_unreachable_at_this_launch` | measured | 2 | 0 |  |
...
### What "144 held" is and is not

The rules were **fitted to these cases.** Six of the sixteen were written from
observations in these very sweeps, so 144/144 is a consistency check, not a
prediction — a model that failed here would simply be broken.

The 60-case corpus is the one that carried information: it was generated from a
seed family the rules had not been fitted to, and it **refuted** the third version
of the ordering rule. The fourth version then held on all 60, and on all 144.

But that is the last honest thing this pair of corpora can say, because the
held-out set is now part of the fit. Validating the fourth version needs a corpus
it has never seen — depth 3, another seed family, or another block size — and the
single experiment most likely to break it is named at the end of finding 2.
