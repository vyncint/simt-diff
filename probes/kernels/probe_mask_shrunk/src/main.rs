//! ORACLE: KNOWN_MASK_INVALID. All 32 lanes are present and call the
//! collective, but the supplied mask names only lanes 0..15. Lanes 16..31
//! participate while unnamed -- the `shrinkmask` class, which
//! conformance/MUTATION.md records at 0% static recall in v1.
use cuda_core::{CudaContext, DeviceBuffer, LaunchConfig};
use cuda_device::{DisjointSlice, cuda_module, kernel, thread, warp};

const KERNEL: &str = "probe_mask_shrunk";
...
mod kernels {
    ...
    pub fn probe(mut out: DisjointSlice<u32>) {
        ...
        let b = warp::ballot_sync(0x0000_ffff, true);
