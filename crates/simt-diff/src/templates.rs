...
impl Template {
    ...
    pub fn kernel_source_for(&self, launch: Launch) -> String {
        render_kernel_file(self.doc, self.extra_uses, self.extra_items, self.body, launch)
    }
...
/// The one place the generated file's layout is written down, so a hand-written
/// template and a generated mutant are the same kind of artifact byte for byte.
///
/// The launch contract is not cosmetic. Without it reconverge emits RC005
/// ("calls `index_1d()` without a launch contract"), which is correct and would
/// be noise in every generated case; declaring it also makes the launch a
/// property of the program the analyzer sees, not just of the runner.
pub fn render_kernel_file(
    doc: &str,
    extra_uses: &[&str],
    extra_items: &str,
    body: &str,
...
/// SHA-256 over arbitrary bytes, for callers outside this module that must
/// hash a case the same way (the mutation engine).
pub fn sha256_hex_public(input: &[u8]) -> String {
    sha256_hex(input)
}
